...
async def check_routing(state: LegalState) -> dict:
    ...
    needs_tax = any(kw in question_lower for kw in ["tax", "irs", "thuế", "evasion", "offshore", "fbar"])
    needs_compliance = any(kw in question_lower for kw in ["compliance", "sec", "sox", "regulation", "aml", "fcpa"])
    needs_privacy = any(kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "ccpa", "consent"])
...
def route_to_specialists(state: LegalState) -> list[Send]:
    ...
    if any(kw in question_lower for kw in ["tax", "irs", "thuế", "evasion", "offshore", "fbar"]):
        tasks.append(Send("call_tax_specialist", state))

    if any(kw in question_lower for kw in ["compliance", "sec", "sox", "regulation", "aml", "fcpa"]):
        tasks.append(Send("call_compliance_specialist", state))

    if any(kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "ccpa", "consent"]):
...
async def call_privacy_specialist(state: LegalState) -> dict:
    """Privacy specialist sub-agent focused on GDPR and data protection law."""
    from langgraph.prebuilt import create_react_agent
    ...
    privacy_prompt = (
        ...
        "privacy regulations, data subject rights, consent requirements, and cross-border "
        "data transfers. Analyse the privacy and data protection aspects of the question. "
